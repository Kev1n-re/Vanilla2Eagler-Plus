...
export class ProxyManager extends EventEmitter {
  ...
  getProcesses() {
    ...
    const worldRows = [...this.worlds.values()].map((w) => {
      ...
      return {
        ...
        mode: "world",
        target: `${st.targetHost || "127.0.0.1"}:${st.targetPort} <- ${st.relayUrl} room=${st.name} code=${codes}`,
        username: st.name,
  ...
  async startWorld(config = {}) {
    ...
    const relayUrl = String(config.relayUrl || cfg.relay?.url || "").trim();
    if (!relayUrl) throw new Error("未配置中继地址");
    const normalized = relayUrl.replace(/\/+$/, "").toLowerCase();
    for (const existing of this.worlds.values()) {
      const other = String(existing.relayUrl || "").replace(/\/+$/, "").toLowerCase();
      if (other === normalized) throw new Error("该中继已有正在运行的房间，一个中继只能开一个房间");
    }
    const world = new EaglerRelayServer({
      ...
      enableSelfTest: !!config.enableSelfTest,
      reconnectDelaySeconds: Number(config.reconnectDelaySeconds) || 10,
      rateLimitRetryDelaySeconds: Number(config.rateLimitRetryDelaySeconds) || 60,

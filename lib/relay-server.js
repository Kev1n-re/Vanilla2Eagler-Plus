...
async function isSafeSkinUrl(url) {
    ...
    this.socket = net.connect({ host: "127.0.0.1", port }, () => {
      try {
        if (server.proxyProtocol && this.peer.remoteIp) {
          ...
          this.socket.write(`PROXY TCP4 ${this.peer.remoteIp} 127.0.0.1 ${srcPort} ${port}\r\n`, "ascii");
        }
      } catch (_) {}
...
class WorldPeer {
  constructor(owner, id) {
    ...
    this.skinRequestWindow = Date.now();
    this.skinRequestCount = 0;
  }
      ...
      if (typIdx > 0 && parts.length > typIdx + 1) {
        ...
        if ((type === "srflx" || type === "prflx") && parts.length > 5) {
          ...
          if (ip.includes(".") && !ip.includes(".local") && Number.isInteger(port) && port > 0) {
            ...
            const server = this.owner.server;
            let sameIp = 0;
            for (const other of this.owner.peers.values()) {
              if (other !== this && other.remoteIp === ip) sameIp++;
            }
            if (sameIp >= server.maxPeersPerIp) this.disconnect();
          }
        }
      }
    } catch (_) {}
  ...
  disconnect() {
    ...
    const server = this.owner.server;
    if (this.username) {
      server.skinsByName.delete(this.username.toLowerCase());
      for (const [uuid, name] of server.namesByUuid) {
        if (name.toLowerCase() === this.username.toLowerCase()) server.namesByUuid.delete(uuid);
      }
    }
    server.emit("peers");
  }
...
class WorldRoom {
  ...
  createPeer(id) {
    ...
    peer.loginTimer = setTimeout(() => {
      if (!peer.username && !peer.disconnected) peer.disconnect();
    }, this.server.loginTimeoutMs);
...
export class EaglerRelayServer extends EventEmitter {
  constructor(options = {}) {
    ...
    this.maxPeersPerIp = 8;
    this.maxSkins = 512;
    this.maxDownloaded = 256;
  ...
  cacheDownloaded(uuidHex, response) {
    if (this.downloadedByUuid.size >= this.maxDownloaded) {
      const first = this.downloadedByUuid.keys().next().value;
      if (first) this.downloadedByUuid.delete(first);
    }
  ...
  registerLoginSkin(raw, peer) {
    try {
      ...
      if (peer.loginTimer) {
        clearTimeout(peer.loginTimer);
        peer.loginTimer = null;
      }
      ...
      if (isMySkinPacket(skinPacket) && this.skinsByName.size < this.maxSkins) {
  ...
  handleSkinMessage(raw, peer) {
    try {
      ...
      const now = Date.now();
      if (now - peer.skinRequestWindow > 10000) {
        peer.skinRequestWindow = now;
        peer.skinRequestCount = 0;
      }
      peer.skinRequestCount++;
      if (peer.skinRequestCount > 20) {
        peer.disconnect();
        return;
      }
      const type = data[0];
      if (type === 0x01 || type === 0x02) {
        if (isMySkinPacket(data) && this.skinsByName.size < this.maxSkins) {
  ...
  async downloadSkin(peer, uuidHex, url) {
    ...
    try {
      ...
      response[17] = 0;
      pixels.copy(response, 18);
      this.cacheDownloaded(uuidHex, response);
      peer.sendLanPacket(buildPluginMessage(response));
    } catch (_) {
      this.sendFallback(peer, uuidHex);
    } finally {
  ...
  rewritePlayerList(raw) {
    try {
      ...
      for (let i = 0; i < count.value; i++) {
        ...
        const name = readMCString(raw, off);
        off = name.next;
        if (this.namesByUuid.size >= this.maxSkins) {
          const first = this.namesByUuid.keys().next().value;
          if (first) this.namesByUuid.delete(first);
        }
